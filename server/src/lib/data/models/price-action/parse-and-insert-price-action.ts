 ...
 * instead of the API.
 */
export const fetchParseAndInsertGroupedDaily = query(
	async (sql, { date }: { date: YearMonthDay }) => {
		...
		await fs.mkdir(new URL(`./parsed-data`, import.meta.url), {
			recursive: true,
		});
		const path = fileURLToPath(
			new URL(`./parsed-data/${date}.json`, import.meta.url)
		);
		await fs.writeFile(path, JSON.stringify(parsedData, null, 0), {});

		const priceAction = await insertPriceAction({ priceAction: parsedData });
		return priceAction;
	}
);
