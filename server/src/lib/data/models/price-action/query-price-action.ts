...
export const queryPriceActionFlat = query(
	async (
		sql,
		{
 ...
 * unix ms values. */
export const queryPriceActionGrouped = query(
	async (
		sql,
		{
			limit = 1e4, // 10_000 row limit by default to prevent accidentally fetching 100 million rows. 😀
...
export const queryTimestamps = query(
	async (
		sql,
		{ table = PRICE_ACTION_TABLES.DAILY }: { table?: `${PRICE_ACTION_TABLES}` }
	) => {
